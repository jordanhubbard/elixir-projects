defmodule GameTest do
  ...
  test "new game for known word looks correct" do
    game = Game.new_game("Bollocks")	# Also test that downcase is working
    assert game.turns_left == 7
    assert game.game_state == :initializing
  ...
  test "winning game progresses through all expected states" do
    ...
    Enum.reduce(states, game,
      fn({guess, expected_state}, new_game) ->
	{new_game, _} = Hangman.make_move(new_game, guess)
  ...
  test "losing game progresses through all expected states" do
    ...
    Enum.reduce(states, {game, game.turns_left},
      fn({guess, expected_state}, {new_game, turns_left}) ->
	assert new_game.turns_left == turns_left
	{new_game, _} = Hangman.make_move(new_game, guess)
	assert new_game.game_state == expected_state
	{ new_game, turns_left - 1 }
      end)
  end
end
